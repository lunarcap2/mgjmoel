	...
	function fn_sumbit(){
		...
		if(txtBirth == '') {
			...
			/*if (dtYear < 1940 || dtYear > year){
				alert("입력하신 생년월일의 연도 정보가 유효하지 않습니다.\n(1940~"+year+"년도 사이의 숫자만 입력 가능)");
				return false;
			}

			if (dtYear < 1970 || dtYear > 2006){
				alert("입력하신 생년월일의 연도 정보가 유효하지 않습니다.\n(1970~2006년도 사이의 숫자만 입력 가능)");
				$('#txtBirth').focus();
				return false;
			}
			*/
			...
			if (dtYear == year && dtMonth > month){
				alert("현재일자보다 미래의 날짜로 생년월일 정보가 입력되었습니다.\n다시 확인해 주세요.");
				$('#txtBirth').focus();
				return false;
			}

			if (dtMonth < 1 || dtMonth > 12){
				alert("입력하신 생년월일의 월 단위 정보가 유효하지 않습니다.");
				$('#txtBirth').focus();
				return false;
			}

			if (dtDay < 1 || dtDay > 31){
				alert("입력하신 생년월일의 일자 정보가 유효하지 않습니다.");
				$('#txtBirth').focus();
				return false;
			}
			...
			if (dtMonth == 2) {
				...
				if (dtDay > 29 || (dtDay == 29 && !isleap)){
					...
					alert(dtYear+"년 2월의 마지막 날짜는 "+strFebDay+"일 입니다.");
					$('#txtBirth').focus();
					return false;
				}
			}
						...
						<div class="tb_area">
							<table class="tb">
								...
								<tbody>
										...
										<td>
											<div class="rdi_box">
												<label class="radiobox">
													<input class="rdi" name="gender" value="1" type="radio" style=""><span>남자</span>
												</label>
												<label class="radiobox">
													<input class="rdi" name="gender" value="2" type="radio" style=""><span>여자</span>
												</label>
											</div>
										</td>
									</tr>
									...
									<tr>
										...
										<td>
											<div class="inp">
												<input type="text" id="txtEmail" name="txtEmail" class="txt placehd" onchange="fn_checkMail();" maxlength="100" placeholder="이메일 주소 입력" autocomplete="off">
									...
									<tr>
										...
										<td>
											<div class="inp btn2">
												<div class="sec">
													<input type="text" id="txtPhone" name="txtPhone" class="txt placehd" maxlength="11" placeholder="휴대폰 번호 입력" style="width:65%; padding:0;">
							...
							<div class="privacy">
								<dl>
									...
									<dd>
										<strong>개인정보 수집목적</strong> : <%=site_name%> 로그인 및 구직자 개인 식별, 입사지원 안내, 고지사항 전달,<br>입사 지원자와의 원활한 의사소통, 이벤트 참여자 당첨자 발표 및 마케팅 활용 목적<br>
										<strong>개인정보 수집항목</strong> : 아이디, 비밀번호, 이름, 생년월일, 성별, 최종학력, 이메일, 휴대폰번호<br />
										<strong>개인정보 이용기간</strong> : 행사 기간<br>(사이트 오픈 후 부터 행사 종료시까지 - 서비스 제공에 관한 발주기관과의 계약 이행을 완료하기 위해 회원의 이용정보를 보관할 필요가 있음)<br>
										<strong>개인정보 파기</strong> : 행사 종료 및 행사 사업보고 종료 보고 후 파기 (발주기관 협의 필)
										<p style="font-weight:bold; margin-top:10px;">* 개인정보 수집 및 이용에 대해서는 거부할 수 있으며, 거부 시에는 박람회 부대행사 이용이 불가합니다.<br>(워크넷 가입 시, 워크넷을 통한 구직등록 및 입사지원은 가능합니다.)</p>
									</dd>
								</dl>
